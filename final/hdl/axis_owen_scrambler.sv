...
module axis_owen_scrambler #
  ...
  localparam logic [FRAC_BITS-1:0] K_INDEX  = 32'h27d4eb2f;
  ...
  logic [INDEX_BITS-1:0] sample_idx_reg [2:0];
  ...
  always_ff @(posedge s00_axis_aclk) begin
    if (s00_axis_aresetn == 0) begin
      ...
      if (s00_axis_tvalid) begin
        ...
        sample_idx_reg[0] <= sample_index;
      end

      seed[1] <= seed[0] ^ (sample_id_reg * K_SAMPLE);
      u0_pipe[1] <= u0_pipe[0];
      u1_pipe[1] <= u1_pipe[0];
      sample_idx_reg[1] <= sample_idx_reg[0];

      seed0[0] <= seed[1] ^ (32'd0 * K_DIM);
      seed1[0] <= seed[1] ^ (32'd1 * K_DIM);
      u0_pipe[2] <= u0_pipe[1];
      u1_pipe[2] <= u1_pipe[1];
      sample_idx_reg[2] <= sample_idx_reg[1];

      seed0[1] <= seed0[0] ^ (sample_idx_reg[2] * K_INDEX);
      seed1[1] <= seed1[0] ^ (sample_idx_reg[2] * K_INDEX);
      u0_pipe[3] <= u0_pipe[2];
      u1_pipe[3] <= u1_pipe[2];
