...
async def test_a(dut):
    ...
    for _ in range(N):
        ...
        seed0 = make_lk_seed(PIXEL_ID, SAMPLE_ID, 0, idx)
        u_scr_int = laine_karras_permutation(u_raw_int, seed0)
        
        seed1 = make_lk_seed(PIXEL_ID, SAMPLE_ID, 1, idx)
    ...
    u_scr_py = [owen_scramble_lk(u_sobol_py[n],pixel_id=0,sample_id=0,dim=0,index=n+1)
                        for n in range(N)]
    v_sobol_py = np.array([sobol_value_stateless(1, n, DIR_TABLE) for n in range(1, N+1)])
    v_scr_py = [owen_scramble_lk(v_sobol_py[n],pixel_id=0,sample_id=0,dim=1,index=n+1)
                        for n in range(N)]
   ...
   # ... (assertions passed) ...
    ...
    # Note: Ensure u_scr_py / v_scr_py lists contain standard Python ints, not numpy types
    u_plot_py_norm = [int(x) / (2**32) for x in u_scr_py]
    v_plot_py_norm = [int(x) / (2**32) for x in v_scr_py]
    ...
    sobol_scatter(u_plot_rtl_norm, v_plot_rtl_norm, 
                  ...
                  "owen_scramble_rtl0.png")
                  
    sobol_scatter(u_plot_py_norm, v_plot_py_norm, 
                  "Owen Scrambled Sobol (RTL Output)", 
                  "owen_scramble_rtl.png")
                  
    print("Plots saved to owen_scramble_rtl0.png and owen_scramble_rtl.png")
